$(document).ready(function () {
    ...
    if (!token || token === 'undefined') {
        ...
        $.ajax({
            url: window.location.origin + '/stylish/api/1.0/user/profile',
            method: 'GET',
            headers: {
    ...
    $('#login-form').submit(function (event) { //native sign in listener
        ...
        $.ajax({ //get native sign in token
            url: window.location.origin + '/stylish/api/1.0/user/signin',
            method: 'POST',
    ...
    $('#register-form').submit(function (event) { //native sign up listener
        ...
        $.ajax({ //get native sign up token
            url: window.location.origin + '/stylish/api/1.0/user/signup',
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(requestBody),
            success: function (response) {
